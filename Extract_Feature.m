...
function HISTOGRAM=Extract_Feature(path)
...
for m=1:1
    for n=1:N2-1
        ...
        I_o_degree(m,n) =I(m,n+1)-I(m,n);
        I_90_degree(m,n)=I(m,n)-I(m,n);
    end
end
...
for m=1:N1
    for n=1:N2  
        ...
        if(I_o_degree(m,n) >=0  && I_90_degree(m,n) >= 0)
            I_1_direction(m,n) =1;
        end
        if(I_o_degree(m,n) <0  &&  I_90_degree(m,n) >=0)
            I_1_direction(m,n)=2;
...
% not needed as in matrix I_1_direction it is indicated 
...
for m=2:N1-1
    for  n=2:N2-1 
          ...
          Mgc_tmp=sqrt(I_o_degree(m,n)*I_o_degree(m,n)+I_90_degree(m,n)*I_90_degree(m,n));
           
          Mp_tmp(1,1)=sqrt(I_o_degree(m,n+1)*I_o_degree(m,n+1)+I_90_degree(m,n+1)*I_90_degree(m,n+1));
          Mp_tmp(1,2)=sqrt(I_o_degree(m-1,n+1)*I_o_degree(m-1,n+1)+I_90_degree(m-1,n+1)*I_90_degree(m-1,n+1));
          Mp_tmp(1,3)=sqrt(I_o_degree(m-1,n)*I_o_degree(m-1,n)+I_90_degree(m-1,n)*I_90_degree(m-1,n));
          Mp_tmp(1,4)=sqrt(I_o_degree(m-1,n-1)*I_o_degree(m-1,n-1)+I_90_degree(m-1,n-1)*I_90_degree(m-1,n-1));
          Mp_tmp(1,5)=sqrt(I_o_degree(m,n-1)*I_o_degree(m,n-1)+I_90_degree(m,n-1)*I_90_degree(m,n-1));
          Mp_tmp(1,6)=sqrt(I_o_degree(m+1,n-1)*I_o_degree(m+1,n-1)+I_90_degree(m+1,n-1)*I_90_degree(m+1,n-1));
          Mp_tmp(1,7)=sqrt(I_o_degree(m+1,n)*I_o_degree(m+1,n)+I_90_degree(m+1,n)*I_90_degree(m+1,n));
          Mp_tmp(1,8)=sqrt(I_o_degree(m+1,n+1)*I_o_degree(m+1,n+1)+I_90_degree(m+1,n+1)*I_90_degree(m+1,n+1));          
          for p=1:8
